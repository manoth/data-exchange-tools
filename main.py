...
def _same_directory(path: str, directory: str) -> bool:
    return os.path.normcase(os.path.abspath(os.path.dirname(path))) == os.path.normcase(os.path.abspath(directory))
...
def _ensure_desktop_shortcut():
    ...
    try:
        registered_launcher = ""
        install_state_path = os.path.join(APP_DIR, "installed.json")
        if os.path.isfile(install_state_path):
            try:
                with open(install_state_path, "r", encoding="utf-8") as f:
                    registered_launcher = str((json.load(f) or {}).get("launcher_path") or "")
            except Exception:
                registered_launcher = ""

        # An EXE already placed on the Desktop is its own launcher. Avoid a
        # duplicate shortcut, and clean up only the shortcut generated by us.
        desktop_launcher_exists = registered_launcher and os.path.isfile(registered_launcher) and _same_directory(registered_launcher, desktop)
        if _same_directory(os.path.abspath(sys.executable), desktop) or desktop_launcher_exists:
            if os.path.isfile(shortcut_path):
                with open(shortcut_path, "r", encoding="utf-8", errors="ignore") as f:
                    existing = f.read()
                if existing.startswith("[InternetShortcut]") and f"URL={APP_URL}" in existing:
...
def _require_loopback(request: Request):
    ...
    if client_host not in {"127.0.0.1", "::1", "localhost"}:
        raise HTTPException(status_code=403, detail="runtime endpoint ใช้ได้จากเครื่องนี้เท่านั้น")
...
async def runtime_shutdown(request: Request):
    _require_loopback(request)
    supplied_token = request.headers.get("X-Launcher-Token", "")
    if not supplied_token or not hmac.compare_digest(supplied_token, get_launcher_secret()):
        raise HTTPException(status_code=403, detail="launcher token ไม่ถูกต้อง")
    _schedule_process_exit(0.5)
    return {"success": True, "message": "กำลังปิด service เพื่ออัปเดต"}
